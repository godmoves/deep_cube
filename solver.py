...
class SolverThread(thr.Thread):
        ...
        if togo_phase2 == 0 and slice_sorted == 0:
            ...
            if len(self.solutions) == 0 or (len(self.solutions[-1]) > len(man)):
                ...
                if self.inv == 1:  # we solved the inverse cube
                    ...
                    man[:] = [en.Move((m // 3) * 3 + (2 - m % 3)) for m in man]
            ...
            for m in en.Move:
                ...
                self.search_phase2(corners_new, ud_edges_new,
                                   slice_sorted_new, dist_new, togo_phase2 - 1)
    ...
    def search(self, flip, twist, slice_sorted, dist, togo_phase1):
        ...
        if togo_phase1 == 0:  # phase 1 solved
            ...
            togo2_limit = min(
                self.shortest_length[0] - len(self.sofar_phase1), 11)
            # this precheck speeds up the computation
            ...
            ud_edges = coord.u_edges_plus_d_edges_to_ud_edges[24 *
                                                              u_edges + d_edges % 24]
            ...
            for togo2 in range(dist2, togo2_limit):
                self.sofar_phase2 = []
                self.search_phase2(corners, ud_edges,
                                   slice_sorted, dist2, togo2)
    ...
    def run(self):
        ...
        if self.rot == 0:  # no rotation
            cb = cubie.CubieCube(
                self.cb_cube.cp, self.cb_cube.co, self.cb_cube.ep, self.cb_cube.eo)
        elif self.rot == 1:  # conjugation by 120° rotation
            cb = cubie.CubieCube(
                sy.symCube[32].cp, sy.symCube[32].co, sy.symCube[32].ep, sy.symCube[32].eo)
            cb.multiply(self.cb_cube)
            cb.multiply(sy.symCube[16])
        elif self.rot == 2:  # conjugation by 240° rotation
            cb = cubie.CubieCube(
                sy.symCube[16].cp, sy.symCube[16].co, sy.symCube[16].ep, sy.symCube[16].eo)
            cb.multiply(self.cb_cube)
...
def solve(cubestring, max_length=20, timeout=3):
    ...
    for i in tr:
        th = SolverThread(cc, i % 3, i // 3, max_length,
                          timeout, s_time, solutions, terminated, [999])
    ...
    if len(solutions) > 0:
        for m in solutions[-1]:  # the last solution is the shortest
            s += m.name + ' '
    return s + '(' + str(len(s) // 3) + 'f:{:.5f}s)'.format(end_time - begin_time)
########################################################################################################################
